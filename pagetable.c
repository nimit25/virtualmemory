...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
        ...
        if (coremap[frame].pte->frame & PG_DIRTY){
            evict_dirty_count++; // dirty page needs to be put in the swap disk
			coremap[frame].pte->swap_off = swap_pageout(
                    ((coremap[frame].pte)->frame >> PAGE_SHIFT),
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    pgtbl_entry_t *table_entry = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
    p = &table_entry[PGTBL_INDEX(vaddr)];
    ...
    if (!(p->frame & PG_VALID)){
		...
		coremap[frame].vaddr = vaddr; // for OPT
        if (p->frame & PG_ONSWAP){
            int victim = swap_pagein((unsigned int) frame, (int) p->swap_off);
            if (victim != 0){
                fprintf(stderr, "swap_pagein exited");
                exit(1);
            }
            p->frame = (unsigned int) (frame << PAGE_SHIFT);
            p->frame = p->frame & ~PG_ONSWAP;
            p->frame = p->frame & ~PG_DIRTY; // because the page is no longer dirty
